...
- generic [active] [ref=e1]:
  - generic [ref=e3]:
    - generic [ref=e4]:
      ...
      - paragraph [ref=e6]: Schritt 4 von 4
    - generic [ref=e8]:
      - heading "E-Mail Bestätigung" [level=3] [ref=e9]
      - generic [ref=e10]:
        - paragraph [ref=e11]:
          - text: Wir haben einen Code an
          - strong [ref=e12]: test_1784211692507@web-library.net
          - text: gesendet. Bitte gib den Code hier ein.
        - generic [ref=e13]:
          - generic [ref=e14]: Bestätigungscode
          - textbox "123456" [ref=e15]
      - button "Verifizieren" [ref=e17]:
        - text: Verifizieren
        - img [ref=e18]
  - alert [ref=e21]
```
...
```ts
  ...
  2   | 
  3   | async function getMailTmAccount() {
  4   |   const response = await fetch('https://api.mail.tm/domains');
  5   |   const domains = await response.json();
  6   |   const domain = domains['hydra:member'][0].domain;
  7   |   const email = `test_${Date.now()}@${domain}`;
